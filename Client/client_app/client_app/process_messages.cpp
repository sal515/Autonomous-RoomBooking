		...
		for (auto day : db) {
			for (auto time : day) {
				if (!req_meeting.meetingID.compare(time.at("meetingID"))) {
					...
					meeting::client_update_meeting(db, thisMeeting.meetingDay, 
		...
		for (auto day : db) {
			for (auto time : day) {
				if (!req_meeting.requestID.compare(time.at("requestID"))) {
					meeting::client_update_meeting(db, time.at("day"),
						time.at("time"), json({}));
					checkTrue = false;
					break;
				}
		...
		for (auto day : db) {
			for (auto time : day) {
				if (!req_meeting.requestID.compare(time.at("requestID"))) {
					meeting thisMeeting = meeting::json_to_meetingObj(time);
					//thisMeeting.
					//meeting::client_get_meeting(db, time.at("day"), time.at("time"),)
				}
			}
		}
	}
